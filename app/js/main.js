...
createRestaurantHTML = (restaurant) => {
  ...
  image.setAttribute('data-echo', imgSrc);
...
window.onload = () => {
  ...
  document.addEventListener('keydown', (event) => {
    const reviewsHeader = document.querySelector('nav h1 a');
    const selectNeighborhood = document.querySelector('#neighborhoods-select');

  	if (event.key === 'Tab' && event.getModifierState('Shift') === false) {
  		if (document.activeElement === reviewsHeader) {
      ...
      if (document.activeElement === selectNeighborhood) {
        event.preventDefault();
  			reviewsHeader.focus();
  	  }
    }
